...
# Adapted for French by Jeremy Yeaton - jdyeaton27@gmail.com
# Created:     12 Dec 2014
#-------------------------------------------------------------------------------
...
class PsExperiment:
    def __init__(self, info):
        ...
        stim_file = os.path.join(self.stim_dir, 'stimuli.csv')
        try:
            self.stim_file = open(stim_file, 'w')
            print stim_file
        except Exception, e:
            print 'Desole, impossible d ecrire le fichier: ' + stim_file
            sys.exit(1)
        ...
        # there are 8 blocks, and so 8*8 = 64 stimuli
        contnum = 1
        fillnum = 1
        blocknum = 8
        with open(os.path.join('misc', 'stimuli.txt')) as tsv:
        ...
        for i in range(0, blocknum):
            ...
            block.append(critical2.pop(random.randint(0, len(critical2) - 1)))
            block.append(critical3.pop(random.randint(0, len(critical3) - 1)))
            block.append(critical4.pop(random.randint(0, len(critical4) - 1)))
            for x in range(0, contnum):
                block.append(control.pop(random.randint(0, len(control) - 1)))
    ...
    def runexp(self):
        ...
        for stim in self.stimuli:
            # Play and record
            self.trial(stim)
        self.show_txt('Fin, merci pour votre participation')
        time.sleep(1)
        mouse = self.getMouse()
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    if event.dict['button'] == 1:
                        ...
                        break
                        self.show_txt('                          enregistrement termine- pressez la barre espace pour continuer                          ', 100)
        # Done recording
